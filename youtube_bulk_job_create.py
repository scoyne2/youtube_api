...
CLIENT_AUTH_TOKEN = '/Users/scoyne/Documents/GitHub/lib-global-analytics/Secrets/youtube_bulk_reporting_credentials.json'
...
def get_authenticated_service(args):
    flow = flow_from_clientsecrets(CLIENT_SECRETS_FILE, scope=YOUTUBE_ANALYTICS_MONETARY_READ_SCOPE, message=MISSING_CLIENT_SECRETS_MESSAGE)
    ...
    return build(YOUTUBE_REPORTING_API_SERVICE_NAME, YOUTUBE_REPORTING_API_VERSION, http=credentials.authorize(httplib2.Http()))
...
def get_reporting_job(name):
    result = youtube_reporting.jobs().get(
      body=dict(
        jobId="scoyne_test",
      )
    ).execute()
